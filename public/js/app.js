...
function rendersToDom(datas, whatToDo) {
    ...
    const DomArticle = datas.map(function (data) {
        ...
        let commentDom = "";
        if (whatToDo === "article") {
            btndata = `<button type="button" class="btn btn-danger btn-block m-1 p-1 save " data-id=${data._id}> save</button>`;
        } else {
...
$(document).ready(function () {
        $.get("/articles", function (dataArticles) {
            ...
            if (dataArticles.length === 0) {
                ...
                <div class="card">
                        <div class="card-body">
                            <h3>You have not scrape any news yet. </h3>
                            <p>Click below to scrape new from CNet.com</p>
                            <a  href="/scrape" class="btn btn-primary btn-lg scrape2" >Scrape news From cnet.com</a>
                        </div>
                    </div>`)
            } else {
    ...
    $(".listNews").on("click", function (event) {
    ...
    $("#clearArticle").click(function (e) {
        $.get("/artile/delete/all", function (data) {
            window.location.href = "/";
        })
    })
